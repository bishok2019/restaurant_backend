...
from base.views.views import CustomAPIResponse
...
# class CustomGenericWithBranchListView(ListAPIView):
#     def get_branch_from_permissions(self):
...
#         branch_permissions = request.user.branch_permissions.all().filter(
#             branch__in=self.get_branches()
#         )

    # def get_branches(self, branch):
    #     return [
    #         int(_) for _ in self.request.query_params.getlist(branch) if _.isdigit()
    #     ]

    # def branch_filtering(self):
    #     branch_query_name = getattr(self, "branch_query_name", "")
    #     branches = self.get_branches(branch_query_name) or [0]
    #     return self.queryset.filter(branch__in=branches)

    # def get_queryset(self):
    #     return self.branch_filtering()
